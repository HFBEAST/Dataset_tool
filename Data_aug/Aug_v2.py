...
def update_annotation(json_path, image_shape, flip_type):
    ...
    for shape in data.get('shapes', []):
        if 'points' in shape and len(shape['points']) == 2:
            ...
            flipped_points = flip_bounding_box(original_points, image_width, image_height, flip_type)
            # 确保左上角和右下角
...
def process_images(folder_path):
    ...
    for operation_name, operation in operations:
        ...
        num_selected = max(1, num_images // 3)  # 确保至少选择一张图像
...
if __name__ == "__main__":
    folder_path = r'E:\PJ\GIO\aiba\dataset\archive_dataset(E8A)_checked\det_json_aug'  # 请替换为您的实际文件夹路径
